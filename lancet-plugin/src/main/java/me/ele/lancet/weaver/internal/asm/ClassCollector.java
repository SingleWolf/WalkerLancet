...
import me.ele.lancet.weaver.ClazzData;
...
public class ClassCollector {
    ...
    public ClazzData[] generateClassBytes() {
        ...
        ClazzData[] classDataArray = new ClazzData[mClassWriters.size() + 1];
        int index = 0;
        for (Map.Entry<String, ClassWriter> entry : mClassWriters.entrySet()) {
            classDataArray[index] = new ClazzData(entry.getValue().toByteArray(), getCanonicalName(entry.getKey()));
            index++;
        }
        classDataArray[index] = new ClazzData(originClassWriter.toByteArray(), originClassName);
